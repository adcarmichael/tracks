...
def get_total_climbs_for_route(route_id):
    ...
    for ind, r_id in enumerate(route_id):
        n_climbs[ind] = RouteRecord.objects.filter(
            Q(route=r_id) & (Q(record_type=os) | Q(record_type=cs))).count()
...
def get_route_set_perc_complete(route_set_id, user_id):
    ...
    query = query.filter(Q(routerecord__user=user_id) & (
        Q(routerecord__record_type=os) | Q(routerecord__record_type=cs))).distinct('id')
